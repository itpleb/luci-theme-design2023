...
export const XHR: any = function () {
  ...
  this.abort = function () {
    if (this.busy()) this._xmlHttp.abort();
  };

  this.get = function (url: any, data: any, callback: any) {
    this.reinit();
    ...
    if (code)
      if (url.substr(url.length - 1, 1) == '&') url += code;
      else url += '?' + code;
  ...
  this.post = function (url: any, data: any, callback: any) {
    this.reinit();
  ...
  this.send_form = function (form: any, callback: any, extra_values: any) {
    var code = '';
    ...
    if (typeof extra_values == 'object')
      for (var key in extra_values) code += (code ? '&' : '') + key + '=' + encodeURIComponent(extra_values[key]);
  ...
  this._encode = function (obj: any) {
    ...
    if (typeof obj == 'object') {
      ...
      for (var k in obj) code += (code ? '&' : '') + k + '=' + encodeURIComponent(obj[k]);
...
XHR.get = function (url: any, data: any, callback: any) {
  new XHR().get(url, data, callback);
};

XHR.poll = function (interval: any, url: any, data: any, callback: any) {
  ...
  if (!XHR._q) {
    ...
    XHR._r = function () {
      for (var i = 0, e = XHR._q[0]; i < XHR._q.length; e = XHR._q[++i]) {
        if (!(XHR._t % e.interval) && !e.xhr.busy()) e.xhr.get(e.url, e.data, e.callback);
      }
